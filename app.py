...
# Set page config
st.set_page_config(
    page_title="CEO International Experience Corporate Misconduct",
    page_icon="📊",
    layout="wide",
...
def format_results_table(results, feature_names, selected_controls=None, industry_fe=False, year_fe=False):
    ...
    def add_stars(coeff, pval, stderr):
        ...
        if pval < 0.001:
            ...
            stars = "*"
        return f"{coeff:.2f}{stars} ({stderr:.2f})"
    ...
    # Control variables mapping - check both log and regular versions
    control_var_mapping = {
        'age': 'Age',
        'gender': 'Gender', 
        'narcissism': 'Narcissism',
    ...
    # Add control variables in order, using actual regression results
    control_display_order = [
        'Age', 'Gender', 'Narcissism', 'Market Value of Shares', 'Tenure', 'Duality', 
        'Cash', 'Dividends Issued', 'Capital expenditures', 'Debt Ratio', 
        'Market Capitalization', 'Net Income', 'Performance', 'Board Size', 'Board Insiders'
    ]
    
    for display_name in control_display_order:
        # Find the actual variable name in the results
        actual_var = None
        for var_key, var_display in control_var_mapping.items():
            if var_display == display_name and var_key in coeffs.index:
                actual_var = var_key
                break
        
        if actual_var:
            results_data['Variable'].append(display_name)
            coeff = coeffs[actual_var]
            pval = pvalues[actual_var]
            stderr = std_errs[actual_var]
            formatted_coeff = add_stars(coeff, pval, stderr)
            
            # Progressive model building - simulate 3 different specifications
            # Column 1: Basic controls (Age, Gender)
            if display_name in ['Age', 'Gender']:
                results_data['(1)'].append(formatted_coeff)
            else:
                results_data['(1)'].append('')
            
            # Column 2: Add more controls (Age, Gender, Narcissism, Tenure)  
            if display_name in ['Age', 'Gender', 'Narcissism', 'Tenure']:
                results_data['(2)'].append(formatted_coeff)
            else:
                results_data['(2)'].append('')
            
            # Column 3: Full specification (all selected controls)
            results_data['(3)'].append(formatted_coeff)
    ...
    if 'const' in coeffs.index:
        ...
        # Add constant to all three columns
        results_data['(1)'].append(formatted_coeff)
        results_data['(2)'].append(formatted_coeff)
        results_data['(3)'].append(formatted_coeff)
...
if page == "Abstract":
    st.title("CEO International Experience Corporate Misconduct")
    st.markdown("---")
    ...
    **We created this online tool to help the readers explore the anonymized data and experiment with different models, 
    ...
    st.write("""
    - What is the relationship between CEO international experience and corporate misconduct?
    - How does the institutional context in which CEO gained international experience affect the relationship between CEO international experience and corporate misconduct?
    ...
    **Hami Usta**  
    ...
    with col1:
        ...
        model_type = st.selectbox(
            "Model Specification:",
            ["Random Effects", "Fixed Effects", "Negative Binomial"],
            index=0
        )
    ...
    with col2:
        ...
        with tab1:
            ...
            if results is not None:
                ...
                if 'ceo_international_experience' in results.params.index:
                    ...
                    fig.update_layout(
                        xaxis_title="CEO International Experience",
        ...
        with tab3:
            ...
            with col_left:
                ...
                You can **select a cutoff point above** for the strength of CEO international experience and compare firms' record of misconduct around changes 
                from CEOs with low international experience to CEOs with high international experience.
                
                We focus on the coefficient on CEO International Experience x New interaction, since it is the DiD estimator reflecting the difference in misconduct 
                between high international experience CEO hires as compared to their low international experience predecessors.
                """)
                
                # Add slider for conservative/liberal analysis
                cutoff_value = st.slider("Low Experience ← → High Experience", 0.0, 1.0, 0.5, 0.1, key="cutoff_slider")
                
                # Show the colored bar
                st.markdown(f"""
                <div style="display: flex; height: 20px; margin: 10px 0;">
                    <div style="background-color: #ff6b6b; width: {cutoff_value*100}%; display: flex; align-items: center; justify-content: center; color: white; font-size: 12px;">Low Experience</div>
                    <div style="background-color: #4dabf7; width: {(1-cutoff_value)*100}%; display: flex; align-items: center; justify-content: center; color: white; font-size: 12px;">High Experience</div>
                </div>
            ...
            with col_right:
                ...
                robustness_data = {
                    'Variable': [
                        'Constant', 'New', 'CEO International Experience', 'CEO International Experience*New',
                        'Age', 'Gender', 'Narcissism', 'Market Value of Shares', 'Tenure', 'Duality',
